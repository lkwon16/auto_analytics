...
실제로 다음 해 재무 관련 정정도 발생한" 97건(2건 매칭 실패, 95건 처리) — 신규
스크립트 `pilot_original_xbrl.py`(rcept_no 역추적)·`xbrl_raw_parser.py`(원본
XBRL zip 파서, 신형·구형 taxonomy 컨테이너 구조 둘 다 지원)·
`pilot_original_xbrl_step2.py`(수집+`live_override.py` 재사용 편차 재계산).

**핵심 결과**: 95건 중 89건(93.7%)이 원본 데이터로도 플래그 유지, 델타 중위수 0
