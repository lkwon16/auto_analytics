...
검증할 필요성은 유지됨 — **2026-07-30 실행 완료(아래 §10 참고)**, 단 검증 범위가
진양성 97건에 한정돼 미탐 809건 확장 여부는 여전히 미결정.

**2026-07-29 (이어서)**: 사용자가 `trade_receivables` 5개 후보 태그의 협의/광의
분류를 직접 재확인하는 과정에서(1~3번=협의, 4~5번=광의가 맞는지 질문), §15의
"협의류 3종"에 4번(`ifrs-full_TradeAndOtherReceivables`)을 잘못 포함시켰던
문서 오류를 발견·정정. 그 과정에서 별개의 실제 버그도 찾음 — 4번 태그가 5번
(광의·유동)과 동시 공시되는 4개사 사례에서 4번 값이 5번의 1/20~1/40로 작아
"광의·비유동(장기)" 잔액으로 보이는데, 후보 순서상 4번이 5번보다 먼저 매치돼
잘못된(작은) 값이 채택되고 있었음. `xbrl_mapping.py`에서 4번 태그를 후보에서
제외 후 재실행 — 영향 6/8,933건(0.07%)뿐이라 Lift 1.09→**1.08배**로 미미하게만
변함(§12 결론 불변). `LIMITATIONS.md` §15에 상세 반영, 커밋 완료.

## 10. §17 파일럿: 정정 전 원본 XBRL 재구성 (2026-07-30)

`LIMITATIONS.md` §17에서 미착수로 남겨뒀던 검증 경로(정정 전 원본 XBRL을
`rcept_no` 기반으로 복원해 look-ahead 가설 실측)를 실행함. 대상은 "플래그됐고
실제로 다음 해 재무 관련 정정도 발생한" 97건 — 신규 스크립트
`pilot_original_xbrl.py`(rcept_no 역추적, 정확매칭 95건+fallback 2건으로
**97/97건 매칭**)·`xbrl_raw_parser.py`(원본 XBRL zip 파서, 신형·구형 taxonomy
컨테이너 구조 둘 다 지원, **97/97건 파싱 성공**)·`pilot_original_xbrl_step2.py`
(수집+`live_override.py` 재사용 편차 재계산).

**사용자 요청으로 재감사(2026-07-30 이어서)**: "놓친 거 없는지 다시 확인해줘"
요청에 실제로 두 가지를 찾아 고침 — (1) rcept_no 매칭 실패 2건이 "[첨부추가]"
접두사 때문이었음을 밝혀 fallback 매칭 추가(95→97/97), (2) 12개 필드 중 일부가
결측으로 나온 25건을 확정본(JSON API)과 대조해 "파서가 놓친 것"과 "그 회사가
애초에 공시 안 하는 것"을 구분 — 전체 97건에서 진짜 파서 miss는 **0건**(결측은
전부 확정본에도 동일하게 결측이던 것). 여러 XBRL 축(ifrs-full/dart-gcd) context가
동시에 존재하는 경우도 값 불일치 0건 확인.

**핵심 결과**: 97건 중 91건(93.8%)이 원본 데이터로도 플래그 유지, 델타 중위수 0
(39건은 완전 동일값) — 하지만 이 표본은 전부 "이미 확정본으로도 잡힌" 진양성
사례라, look-ahead 가설의 핵심 주장(확정본에서는 안 잡히는데 원본이었다면 잡혔을
