...
(⑧ 기업 스크리닝 도구는 이 MVP 로드맵과 별개인 스핀오프라 위 표에 포함하지 않음 —
`README.md` "구성(모듈 ①~⑧)" 참고, Notion에도 별도 하위 페이지로 분리돼 있음)

## 3. 현재 코드베이스 상태

> **⚠️ 이 §3은 2026-07-10 시점 기록이며 이후 크게 바뀌었다(오래된 스냅샷).
> 현재 상태는 §6 "진행 현황"(및 §6-1~6-3) 이하를 볼 것 — 폴더 구조·진행 상태
> 체크리스트는 아래 그대로 두되 참고용으로만 취급.**
...
- STEP 4(`backtest.py`) 완료: 라벨=`[기재정정]사업보고서`(반기·분기 제외, 감사보고서 단독
  ...
  - 원인: `LIMITATIONS.md` §6(정정 후 데이터 순환 문제)이 실증된 것으로 판단 — DART 확정
    데이터는 이미 정정 반영된 값이라 탐지 대상인 이상 징후가 데이터 도달 전에 지워져
    있음. 실무에서 raw 당기 데이터로 분석하면(`LIMITATIONS.md` §11) 다를 수 있으나
    검증 불가능한 가설로 남김(`LIMITATIONS.md` §12 상세 — **2026-07-29 갱신: 외부 리뷰로
    이 전제 자체가 재검토됨, `LIMITATIONS.md` §17 신규 참고 — DART가 정정 전 원본을
    아예 제공 안 한다는 전제가 틀렸을 가능성 제기, 검증 파일럿은 미착수**).
  ...
  - **후속 개선 (v2 진행 중 반영)**: 2026-07-19 `trade_receivables` XBRL 매핑 수정으로
    ...
    하다고 보기엔 부족한 수준 — `LIMITATIONS.md` §6 look-ahead 구조적 한계가 근본
    원인이라는 결론은 유효(단, 2026-07-29 신뢰구간 재계산 결과 §12 참고 — 이 소수점
    변동 자체가 통계적으로 유의하지 않을 수 있음).
...
- **검증(2026-07-13)**: 한양이엔지(00216762) 2022년(정상 범위 플래그, 종합
  스코어 9.73 — 이노스페이스 같은 `LIMITATIONS.md` §10 극단값 케이스 아님)을 예시로
  매칭 확인.
...
STEP 4 백테스트 Lift 0.91배(CLAUDE.md §6 진행 현황 참고, 무작위 이하)의 원인을 두 가지
데이터 품질 문제로 좁혀 순차 개선함:
...
- **결론**: 라벨·매핑 품질 개선으로 얻을 수 있는 향상분은 이번 두 조치로 대체로
  소진된 것으로 판단. Lift가 여전히 1.1배 근처에 머무는 것은 `LIMITATIONS.md` §6의
  look-ahead 구조적 한계(DART 확정 데이터는 이미 정정 반영됨) 때문이라는 결론이
  강화됨 — 추가 라벨 튜닝보다 `LIMITATIONS.md` §11(실무용 당기값 입력 override 경로)
  등 v2 항목으로 넘어가는 것이 합리적. **2026-07-29 갱신**: 이 "결론이 강화됨"이라는
  서술 자체가 재검토 대상 — 0.91→1.02→1.09 변동이 신뢰구간 안에서 통계적으로
  유의하지 않을 수 있음(`LIMITATIONS.md` §12 CI 계산 참고), look-ahead 단일 귀속도
  §10 극단값 confound를 배제하지 않은 상태였음(`LIMITATIONS.md` §12·§17 참고).

## 6-3. 실무용 당기값 입력 경로 + 카테고리 매칭 개선 (2026-07-22)

- **`LIMITATIONS.md` §11**: `detect_flags.py`의 편차 산식(기대치 가중결합·IQR 표준화·
  상위 3개 종합 스코어)을 `deviation_for_value()`/`rank_top_contributors()`로
  공용 함수화. 신규 `live_override.py`가 이를 재사용해, 감사인이 당기(미확정) 원본
  계정값 12개(매출액·매출원가·영업이익 등)를 입력하면 peer·자사 전년도는 기존 DART
...
- **`LIMITATIONS.md` §13**: `disclosure_events` 288,150건의 실제 `report_nm` 빈도를
  집계해 미매칭 유형(196,272건, 68.1%)을 실측 감사. 빈도 높고 인과관계 뚜렷한
...
(위 §6-1 참고). 나머지는 CLAUDE.md §5 "v2 업그레이드 경로" 참고 (평가 프레임
이원화 → 회귀 기대모형+동적 임계값 → OFS fallback·분기 데이터 →
isolation forest+SHAP → LLM 리포트 서술). `LIMITATIONS.md` §5(기재정정 라벨 정제)는
2026-07-20 완료됨(위 §6-2 참고). `LIMITATIONS.md` §11(실무용 당기값 입력 경로)·§13
(이벤트 카테고리 정밀도 1차 개선)은 2026-07-22 완료됨(위 §6-3 참고).

**2026-07-29**: 외부 리뷰를 계기로 백테스트 결과 서술의 통계적 엄밀성·look-ahead
귀속 근거를 재검토(위 §6-2 갱신 메모, `LIMITATIONS.md` §12·§17 참고).
(1) 문서 정합성 정리, (2) `backtest.py`에 신뢰구간·유의성 검정 추가(Lift 1.09배는
p=0.19로 무작위와 통계적으로 구분 안 됨 확인), (3) §10 극단값 confound 분리 검증
(극단값 제외 시 Lift 1.09→1.13, p=0.19→0.09로 다가가지만 여전히 유의하지 않음 —
confound는 실재하나 전부는 아님) **완료**. (4) 원본(정정 전) XBRL 재구성 파일럿은
(2)(3) 결과로도 여전히 유의성 미달 잔여분이 남아 look-ahead 가설을 실측으로
검증할 필요성은 유지됨 — 착수 여부·규모는 사용자 확인 후 결정(비용: 신규 XBRL
파서 + `correction_details` 원본 rcept_no 역추적 로직, `LIMITATIONS.md` §17 참고).
