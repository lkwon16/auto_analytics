...
flowchart TD
    A["① 데이터 파이프라인
OpenDART 재무제표·공시 수집"] --> B["② 동종그룹 구성
KSIC 업종코드 기반 peer group"]
    B --> C["③ 재무비율 엔진
XBRL 매핑 → 비율 10종 산출"]
    C --> D["④ 편차 탐지
peer 기대치 대비 편차 스코어링"]
...
회계연도 1/1 ~ 12/31"]
    B --> D["post_year
익년 1/1 ~ 4/30(사업보고서 제출 시한 전후)"]
    C --> E["disclosure_events·news_signals 조회
(두 윈도우 중 하나라도 포함)"]
    D --> E
    E --> F["텍스트에 키워드 카테고리 규칙 적용
(event_categories.categorize)"]
    F --> G{"편차 상위 비율의
관련 카테고리와 겹치는가?"}
