...
```mermaid
flowchart TD
    A["플래그된 기업 + 회계연도 + 편차 상위 1~3개 비율"] --> B["시점 윈도우 계산"]
    B --> C["concurrent<br/>회계연도 1/1 ~ 12/31"]
    B --> D["post_year<br/>익년 1/1 ~ 4/30(사업보고서 제출 시한 전후)"]
    C --> E["disclosure_events·news_signals 조회<br/>(두 윈도우 중 하나라도 포함)"]
    D --> E
    E --> F["텍스트에 키워드 카테고리 규칙 적용<br/>(event_categories.categorize)"]
    F --> G{"편차 상위 비율의<br/>관련 카테고리와 겹치는가?"}
    G -->|Yes| H["relevant = True<br/>(★관련 표시)"]
    G -->|No| I["relevant = False"]
    H --> J["(relevant, 날짜) 순 정렬<br/>→ 편차 원인 후보 목록"]
    I --> J
```
