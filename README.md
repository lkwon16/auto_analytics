...
```mermaid
flowchart TD
    A["기업×연도별 재무비율 10종 (ratios)"] --> D["기대치 = peer 중위수 50% 
    + 자사 전년도 50%"]
    ...
    (실제값 − 기대치)/peer IQR"]
    D --> F
    F --> G["비율 10개 중 |편차| 상위 3개
    평균 = 종합 스코어"]
    G --> H{"종합 스코어가
상위 10%인가?"}
    H -->|Yes| I["is_flagged = 1 
    → flags 테이블"]
    H -->|No| J["플래그 없음"]
```
...
연간 비율 편차는 "회계연도 중 정확히 언제" 발생했는지 특정할 수 없다는 문제가
있어, 편차 시점 ±N개월이 아니라 **회계연도 자체를 매칭 윈도우**로 잡는다
(`LIMITATIONS.md` §8).

```mermaid
flowchart TD
    A["플래그된 기업 + 회계연도 
    + 편차 상위 1~3개 비율"] --> B["시점 윈도우 계산"]
    B --> C["concurrent
회계연도 1/1 ~ 12/31"]
    B --> D["post_year
익년 1/1 ~ 4/30(사업보고서 
제출 시한 전후)"]
    C --> E["events·news_signals 조회
(두 윈도우 중 하나라도 포함)"]
    D --> E
    E --> F["텍스트에 키워드 카테고리 
    규칙 적용"]
    F --> G{"편차 상위 비율의
관련 카테고리와 겹치는가?"}
    G -->|Yes| H["relevant = True
(★관련 표시)"]
    G -->|No| I["relevant = False"]
    H --> J["(relevant, 날짜) 순 정렬
→ 편차 원인 후보 목록"]
    I --> J
```

수시공시(`report_nm`)는 정형화된 문구라 카테고리 매칭 정확도가 높은 반면, 뉴스
제목·본문은 자유 문장이라 오탐/누락 가능성이 더 크다(`LIMITATIONS.md` §14). 뉴스는
온디맨드로만 수집되므로(대시보드에서 감사인이 조회할 때) 이 함수 호출 시점에 아직
없을 수 있다 — 그 경우 수시공시만으로 결과를 반환한다.
...
| `backtest_labels` | 9,038개(기업×연도) | Precision@10% 10.80%(95% CI [8.77%,12.83%]), Lift **1.08배**(95% CI [0.88,1.28], p=0.22) — 무작위와 통계적으로 구분 안 됨, 원인은 `LIMITATIONS.md` §12 |
