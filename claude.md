...
1. 수정판 `collect_disclosures.py` 실행 결과 확인 (분기별 적재 건수 + 마지막 출력되는 기재정정 건수). 에러 시 status/message 확인
2. 재무제표 수집 완료 검증: `financial_statements`의 기업 수·행 수·연도 커버리지 확인 쿼리
3. STEP 1: 업종코드 수집 스크립트(`collect_industry_codes.py`) 작성 — company.json API, corp_master.industry_code UPDATE, 재실행 안전하게. 단 재무제표 수집과 같은 날 실행 금지 (API 한도)
4. STEP 1: `analysis_universe` 정의 쿼리/스크립트 + 중복 체크 + LIMITATIONS.md
5. 이후 STEP 2 비율 엔진부터 순차 진행

## 7. 문서 산출물 (노션에 정리되어 있음)
