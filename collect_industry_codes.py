"""상장사 업종코드(industry_code) 수집 → corp_master 업데이트
API: company.json (기업개요) — corpCode.xml에는 업종코드가 없어 회사별로 조회 필요
...
def fetch_induty_code(corp_code: str) -> str | None:
    ...
    return (data.get("induty_code") or "").strip() or None
...
def main():
    ...
    with engine.connect() as conn:
        corps = conn.execute(text(
            "SELECT corp_code FROM corp_master WHERE industry_code IS NULL"
        )).fetchall()

    if not corps:
        print("업종코드가 없는 기업이 없습니다. (이미 전부 수집됨)")
        return

    print(f"대상 {len(corps):,}개")
    updated = 0
    with engine.connect() as conn:
        for (corp_code,) in tqdm(corps, desc="industry_code"):
            try:
            ...
            if induty_code:
                with conn.begin():
                    conn.execute(
                        text("UPDATE corp_master SET industry_code = :ic WHERE corp_code = :cc"),
                        {"ic": induty_code, "cc": corp_code},
                    )
                updated += 1
            time.sleep(0.1)

    print(f"업종코드 수집 완료 - {updated:,}건 업데이트")
